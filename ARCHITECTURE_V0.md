...
**Where linkage now lives.** The per-question secrets live in a **separate, short-retention Postgres instance** the broker owns — never in the shared DB that holds envelopes/registrations. RDS backup retention is instance-wide, so this separation is what makes "destroy at close" a *real* deletion horizon rather than one silently inheriting the main DB's long retention (ADR-098). Re-linking the answers table to individuals requires *both* a question's **still-live** `s_q` *and* the `registrations` nullifier list — and is impossible for any question whose secret has been destroyed. Per-person tallies the broker genuinely needs (the §14.2 ask-credit count, the "respondents" stat) are kept as explicit counters on the `registrations` row, not derived by scanning answers. The broker can still link when it must (revoke, on-chain invalidation, gating) **for live questions only**; the bulk data — the thing most likely to be queried, exported, or leaked — carries only per-question pseudonyms.
...
The composite primary key on `envelopes(question_id, unique_identifier)` is the hard enforcement of Sybil resistance at the database layer. Because `unique_identifier` holds the deterministic per-question voter tag (§1.4), a repeat answer from the same human to the same question collides on the PK and is rejected — the broker can crash, restart, double-submit and the DB still rejects duplicates — while two answers by the same human to *different* questions store unrelated tags, so the table cannot be grouped back into one person's history. Each question's secret `s_q` that derives the tag lives in a **separate, broker-owned, short-retention Postgres instance** (the `question_secrets` table, ADR-098), never in this shared database, and is destroyed a grace period after the question closes; per-person tallies are kept on `registrations.{answer_count,signal_count}` rather than by scanning `envelopes`.

> **Secrets store (ADR-098).** `question_secrets(question_id, secret, closes_at, destroyed_at)` is **not** part of this shared schema. It lives on a distinct Postgres instance the broker owns, reachable only by the broker (`HEARME_BROKER_SECRETS_DATABASE_URL`), with short backup retention — its retention is the unlink horizon. The instance separation is required because RDS automated backups are instance-wide: co-locating the secrets with the envelopes would silently make the main DB's long retention the secret's deletion horizon. A periodic reaper destroys (`secret = NULL`) every secret whose question closed more than the grace window ago.
...
parse (pydantic)
  ...
  → ensure (lazily mint) this question's secret s_q in the secrets instance, then
...
- **Linkage-secret management (§1.4, ADR-098) — resolved.** Each question's secret `s_q` lives in a separate broker-owned short-retention Postgres instance and is destroyed a grace period after the question closes (a reaper keyed on `closes_at`). Accepted consequence: revoke / Self-invalidation reach only **live** questions — once `s_q` is destroyed a closed question's answers can no longer be located (their aggregate is already published, so this is correct). Open knobs: the grace window length and the secrets-instance retention (jointly the unlink horizon); KMS-wrapping `s_q` as optional at-rest hardening; and whether to ever move the secrets off RDS for a stronger real-deletion guarantee (ADR-098 Alternatives).
