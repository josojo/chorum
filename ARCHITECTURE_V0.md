...
- **Verify envelope** — happy path + expired token, revoked/unknown registration, bad agent signature, swapped `question_id`/`answer`/`nonce`/`delegation_hash` each reject. **No bridge call** on this path (asserted).
