# Hearme — v2 Architecture (bigger incentives, made safe)
...
- **Phone-held voting authority for protected questions.** For bribery-sensitive civic questions, the agent drafts but does not hold final voting authority. A small Hearme phone app creates the MACI/voting key inside Secure Enclave / StrongBox, binds its public key to the Self nullifier at registration, and signs off the agent's prepared answer batches. MACI becomes enforceable again: the briber can't buy a one-shot key file and run forever; they need ongoing cooperation from the user's identity device, while still lacking a receipt of how the user ultimately voted.
...
For users who want higher weight and payout, Hearme offers an optional model gateway: a **conversation-provenance proxy** in front of OpenRouter or another router. The user's agent sends normal model calls to the proxy; the proxy runs in a remotely attested TEE, forwards to the chosen provider, and **never persists plaintext**. Per turn it stores only a salted/nonce-separated commitment chain:
```
...
```
For a question created at `t_q`, only proxy spend anchored before `t_q` counts toward the cap for that question's rewards. Because `alpha < 1`, a fabricated persona that exists only to farm Hearme must spend more on pre-question conversations than it can withdraw — the fake history can exist, but it is a loss-making influence campaign, not a profitable farm.

- **Honest user:** already uses an agent through the proxy; Hearme rewards a portion of real, pre-existing model use with higher weight and `β`.
...
Two non-negotiable privacy constraints: commitments must be salted/nonce-separated so short opinions ("yes"/"no") aren't dictionary-attackable from stored hashes; and the proxy must be **optional and tier-improving, not mandatory** — routing all personal-agent traffic through Hearme would violate the consent/data-sovereignty spirit of v0 §1.1–§1.2.
...
- **Epoch-rotated scopes (privacy upgrade).** Replace `scope="hearme-v1"` with `scope="hearme-epoch-<N>"` rotating monthly (still ≤31 ASCII), so the broker can no longer link a user's answers across epochs. Phone issues a small batch of epoch tokens at install; Self derives a fresh nullifier per scope. Interacts with the per-identity history that tiers (§8) and longitudinal re-asking (§3.1) depend on — needs a linking-token design that preserves history without preserving cross-epoch answer-linkability.
