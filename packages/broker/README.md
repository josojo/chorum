...
agent binds into `agent_signature`. **Already present** in the schema
(defined in `packages/web/src/db/schema.ts`, generated into
...
and apply the schema (the generated migrations under
`packages/web/drizzle/migrations/`, plus the pgcrypto extension).
