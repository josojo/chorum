...
// CHORUM_BROKER_PRODUCTION_MODE=1, server.ts calls enforceProductionConfig before
...
export function validateProductionConfig(settings: Settings): ValidationReport {
  ...
  if (settings.brokerSigningKey === DEV_BROKER_SIGNING_KEY) {
    report.errors.push(
      "CHORUM_BROKER_SIGNING_KEY is the documented dev default. Anyone with the " +
  ...
  if (settings.voterTagMasterKey === DEV_VOTER_TAG_MASTER_KEY) {
    report.errors.push(
      "CHORUM_BROKER_VOTER_TAG_MASTER_KEY is the documented dev default. It wraps the " +
  ...
  if (settings.secretsDatabaseUrl === DEV_SECRETS_DATABASE_URL) {
    report.errors.push(
      "CHORUM_BROKER_SECRETS_DATABASE_URL is the documented dev default. Point it at the " +
        "broker-owned chorum_secrets database (ADR-098); see docs/DEPLOYMENT.md.",
    );
  }

  if (settings.databaseUrl.includes("chorum_broker_dev")) {
    report.errors.push(
      "CHORUM_BROKER_DATABASE_URL still uses the dev password ('chorum_broker_dev'). " +
  ...
  if (settings.devInsecureRegister) {
    report.errors.push(
      "CHORUM_BROKER_DEV_INSECURE_REGISTER=1 mounts POST /v1/dev/register which mints " +
  ...
  if (!settings.requireRegistryConfirmation) {
    report.errors.push(
      "CHORUM_BROKER_REQUIRE_REGISTRY_CONFIRMATION=0 skips the one-time on-chain Celo " +
  ...
  if (settings.exposeRejectionReasons) {
    report.errors.push(
      "CHORUM_BROKER_EXPOSE_REJECTION_REASONS=1 makes the broker a verification oracle " +
  ...
  // CHORUM_BROKER_SELF_SCOPE there — config.ts / verify/scope.ts). If an operator
  // set the env var expecting it to apply, warn that it is inert (GH #97).
  const rawScope = process.env.CHORUM_BROKER_SELF_SCOPE;
  if (rawScope && rawScope !== PRODUCTION_SCOPE) {
    report.warnings.push(
      `CHORUM_BROKER_SELF_SCOPE='${rawScope}' is IGNORED in production — the broker scope is ` +
  ...
  if (
    ...
    report.warnings.push(
      `CHORUM_BROKER_SELF_BRIDGE_URL='${settings.selfBridgeUrl}' points at localhost. ` +
