...
///
/// `hearme_root` is the data root (`<profile-home>/hearme`) this shim is being
/// installed into. When present it is baked in and passed to every binary
/// invocation as `HEARME_SKILL_ROOT_DIR`, so the answering subprocess always
...
/// `<home>/plugins/hearme` layout, in which case the shim falls back to the
/// runtime `HEARME_SKILL_ROOT_DIR`/`HERMES_HOME` resolution.
pub fn build_subprocess_shim(binary_path: &str, hearme_root: Option<&str>) -> String {
...
# The profile data root (`<profile-home>/hearme`) this shim was installed into,
...
def _skill_env():
    ...
    # fall back to the global ~/.hermes/hearme (wrong delegation token + ledger).
    # An explicit HEARME_SKILL_ROOT_DIR already in the environment wins, so an
    # operator can still override.
    env = os.environ.copy()
    if HEARME_ROOT and not env.get("HEARME_SKILL_ROOT_DIR"):
        env["HEARME_SKILL_ROOT_DIR"] = HEARME_ROOT
    return env
...
pub fn install_plugin_dir(
    ...
    // Pin the shim to the profile it lives in: target is `<home>/plugins/hearme`,
    // so its grandparent is the Hermes home and `<home>/hearme` is the data root.
    // Off-standard layouts (no two parents) bake nothing and fall back to the
    // shim's runtime resolution.
    let hearme_root = target
        .parent()
        .and_then(|plugins| plugins.parent())
        .map(|home| home.join("hearme").to_string_lossy().into_owned());
    std::fs::write(
        target.join("__init__.py"),
        build_subprocess_shim(&resolved_bin, hearme_root.as_deref()),
    )?;
    Ok(target)
}
...
mod tests {
    ...
    fn shim_pins_installed_root_into_subprocess_env() {
        // A baked root must be passed to the binary as HEARME_SKILL_ROOT_DIR so a
        ...
        let shim = build_subprocess_shim(
            "/opt/hearme/bin/hearme-skill",
            Some("/home/u/.hermes/profiles/anita/hearme"),
        );
        assert!(shim.contains("HEARME_ROOT = \"/home/u/.hermes/profiles/anita/hearme\""));
        assert!(shim.contains("def _skill_env():"));
        assert!(shim.contains("env=_skill_env()"));
        assert!(shim.contains("env[\"HEARME_SKILL_ROOT_DIR\"] = HEARME_ROOT"));
    }
    ...
    fn shim_without_root_bakes_empty_and_keeps_runtime_fallback() {
        ...
        let shim = build_subprocess_shim("/opt/hearme/bin/hearme-skill", None);
        assert!(shim.contains("HEARME_ROOT = \"\""));
        assert!(shim.contains("os.environ.get(\"HERMES_HOME\")"));
    }

    #[test]
    fn install_plugin_dir_bakes_profile_root_from_target() {
        // The shim's baked root is the sibling `hearme` dir of the plugins tree
        ...
        let plugin_target = dir
            ...
            .join("plugins")
            .join("hearme");
        install_plugin_dir(Some(&plugin_target), Some("/opt/hearme/bin/hearme-skill")).unwrap();
        let shim = std::fs::read_to_string(plugin_target.join("__init__.py")).unwrap();
        let expected_root = dir.path().join("profiles").join("anita").join("hearme");
        assert!(shim.contains(&format!("HEARME_ROOT = \"{}\"", expected_root.display())));
    }
