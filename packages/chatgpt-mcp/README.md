...
DNS/TLS, an OAuth client and callback URL, a production database migration for
the encrypted user store, KMS/secret-manager configuration, and the deployed
Self bridge callback URL. The endpoint must be registered/configured as a
ChatGPT app. None of those deployment or external-vote steps are performed by
this package.
