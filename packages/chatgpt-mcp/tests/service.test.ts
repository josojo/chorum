...
test("OAuth binds a one-use PKCE code to the verified Self subject", async () => {
  ...
  const oauth = new ChorumOAuthServer(service, "https://chorum.test", "chatgpt", new Set(["https://chatgpt.test/callback"]));
  ...
  const authorizeUrl = new URL("https://chorum.test/oauth/authorize?client_id=chatgpt&redirect_uri=https%3A%2F%2Fchatgpt.test%2Fcallback&response_type=code&code_challenge_method=S256&code_challenge=" + encodeURIComponent(pkceChallenge(verifier)) + "&state=xyz");
  await oauth.authorize(authorizeUrl, response());
  const encodedRequest = /request=([^&\"]+)/.exec(body)?.[1];
  const pending = new URL(`https://chorum.test/oauth/authorize/status?request=${encodedRequest ?? "missing"}`);
  const request = new URLSearchParams(pending.search).get("request");
  assert.ok(request);
  ...
  await oauth.token(new URLSearchParams({ grant_type: "authorization_code", code, code_verifier: verifier }), tokenResponse);
