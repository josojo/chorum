# Chorum Identity Model — Self (self.xyz)

> Identity is the foundation of Chorum. Without a credible "one human, one voice" guarantee, every aggregate signal the platform produces is worthless.

> **Decision (2026-05):** Chorum's proof-of-personhood is built on **Self ([self.xyz](https://self.xyz))**, the single identity provider. An earlier design used **zkPassport**; we switched after weighing adoption and longevity (Self: 8–15M users, Google Cloud / Opera / Celo backing; zkPassport: niche, "not production-ready for critical apps" per Safe research) *and* confirming Self's SDK preserves Chorum's three non-negotiables — off-chain verification at answer time (the only chain read is a one-time registry check at registration), in-proof agent-key binding, and a stable per-scope nullifier. zkPassport is retained below only as **alternatives-considered** context. The concrete wire format and flow live in `ARCHITECTURE_V0.md` §8.

## The Problem

Chorum's political signal is only meaningful if each verified human counts once. Two failure modes are equally fatal:
...
The fix is architectural: **the vote belongs to a Chorum identity, and a Chorum identity can only be created by presenting a unique-personhood proof at registration.** Other credentials (X, GitHub, ENS) can be attached later as reputation metadata, but they never create a new voting identity.

This document describes how **Self** is used as the unique-personhood proof that gates Chorum account creation (and why zkPassport was considered and dropped).
...
Combining this with zero-knowledge proofs lets a user prove "I hold a valid passport from country X, I am over 18, and here is a Chorum-scoped nullifier" — **without revealing the passport itself, the document number, or their name.**
...
- A **scope-bound nullifier**: a deterministic, *unique-per-user-per-scope* value. Under Chorum's scope `"chorum-v1"` the same passport always yields the same nullifier, which is Chorum's `unique_identifier`.

**What it does not reveal:** passport number, name, photo, and — in Chorum's configuration — exact date of birth (we use age *thresholds*, never DOB; see below).

**Why Self clears Chorum's three non-negotiables:**
...
| **Stable per-scope unique identifier** | The nullifier is `unique-per-user-per-scope`; `scope="chorum-v1"` gives one stable identifier per passport across all Chorum answers. |

**How Chorum uses it (registration):**

1. User installs the Chorum skill; it generates an Ed25519 `agent_key`.
2. The self-bridge builds SelfApp request(s) — `scope="chorum-v1"`, `endpoint=<bridge callback>`, `userDefinedData=hex(agent_key)`, disclosures `{nationality, minimumAge}` — and returns universal-link/QR URLs.
...
4. The skill posts the verified proofs (the **enrollment bundle**) to the broker's `POST /v1/register`. The broker runs `SelfBackendVerifier.verify()` via the bridge, checks the nullifier is unseen in Chorum's registry and that `userDefinedData == agent_key`.
5. If valid and unique, a Chorum identity is created, bound to the nullifier. Disclosed attributes are **bucketed** (below) into demographic metadata; raw values are not persisted. The broker then issues a **broker-signed session credential** (the `DelegationToken`) that the agent replays per answer — **the Self proof is verified once here and never again.** This is forced by Self's ±1 day proof-freshness window, and it means no raw proof or raw country crosses the boundary at answer time (ARCHITECTURE §1.2, §5, §8).

**Demographic disclosure — how Chorum reconstructs `age_band` and `region`.** Self has no native "5-year band" or "region" predicate, so Chorum derives both:

- **Region** ← disclosed `nationality` (ISO-3166 country) → mapped to `EU` / continent and **bucketed before storage**.
- **Age band** ← a **multi-threshold ladder**: at install the user runs several `older-than` proofs at thresholds `[18, 25, 35, 50, 65]`, all under `scope="chorum-v1"` so they share one nullifier. The set of passing thresholds reconstructs a band (e.g. `older_than(35)=T ∧ older_than(50)=F → "35-49"`). **Exact DOB is never disclosed.** Only the `18+` proof is required; the finer ones are optional (a user who skips them gets `age_band="18+"`).

The nullifier is the load-bearing piece: deterministic in `(passport, scope)`, so the same passport scanned twice produces the same nullifier — and Chorum rejects the second registration.

> **⚠️ The scope is FROZEN. Never change it in production.** Because the nullifier is deterministic in `(passport, scope)`, changing the scope gives *every existing user a brand-new identity*: Sybil resistance resets, every `registrations` row orphans, and every per-question voter tag stops matching — with **no migration path** (old nullifiers cannot be re-derived under a new scope). Production is pinned to `chorum-v1` **in code** (`packages/self-bridge/src/scope.js` → `PRODUCTION_SCOPE`): when `SELF_PRODUCTION_MODE=1` the bridge ignores any `SELF_SCOPE` env var, so a stray config change cannot silently re-mint identities. Staging uses a separate frozen scope (`staging-chorum-v1`) so its mock-passport identities never collide with mainnet. See `docs/DEPLOYMENT.md` §3.1 "Frozen constants — never change in prod" (GH #97).
...
**Why not zkPassport.** It was the prior choice (its nullifier mapped cleanly onto `unique_identifier` and `fast` mode verified locally). But it is niche — Safe's research calls it "not production-ready for critical applications" — and a small project being abandoned would break Chorum's identity layer. Self preserves the same three architectural wins while bringing materially more adoption, funding (Google Cloud / Opera / Celo), and document coverage.

**Why not run both.** Offering two providers reintroduces a cross-system Sybil hole: the *same passport* yields *different* nullifiers under different SDKs, so one person could register twice (once per provider). Closing that needs either a shared canonical nullifier derivation (cross-project coordination, not available today) or a single-provider-per-country rule (operational complexity). A single provider eliminates the hole by construction. If Self ever needs a fallback, the clean path is to add a second provider *together with* a shared `hash(DG1 || "chorum")`-style nullifier so both dedup against one registry — a roadmap item, not v0.


---

## What the Chorum Identity Looks Like After Registration

After successful passport verification, a Chorum identity consists of:

- **Nullifier** — the Self `unique-per-user-per-scope` identifier (scope `"chorum-v1"`). Used to prevent re-registration. Never displayed.
...
- **Reputation stamps** (optional, attached later) — X account, GitHub, ENS, Lens, prior Chorum participation. These never create new voting rights; they only feed a reputation score that may be displayed alongside aggregates or used for weighted sampling.
...
1. **Not everyone has a biometric passport.** Coverage skews toward wealthier and more travel-active populations. Chorum's demographic breakdowns will reflect this skew until additional unique-personhood proofs (Aadhaar via Anon Aadhaar, national ID schemes, social-graph systems for the unbanked) are added.
...
3. **Stolen passports.** A stolen passport with a known PIN-equivalent can register a Chorum identity for someone other than the rightful holder. The rate is low but nonzero; reporting and revocation mechanisms are needed.

4. **State-level attacks.** A government with control over its CSCA could issue fake passports and create fake Chorum identities at scale. Mitigations: monitor per-country registration rates, flag anomalies, optionally weight or cap per-country participation in any single question.

5. **Off-chain verification trusts the bridge — anchored by a registration-time on-chain check.** Self's `SelfBackendVerifier` SNARK-checks proofs off-chain. To stop a forged/stale Merkle root (which would let a fabricated identity slip through), at registration the broker also reads Self's **on-chain Celo Identity Registry once** to confirm the proof's root is current and the identity is registered (`SELF_CELO_RPC_URL` / `registryConfirmed`; ARCHITECTURE §5). Residual: this read happens only at registration, so a Celo-side revocation made *afterward* isn't reflected — Chorum's own `registrations` registry governs revocation from then on. The bridge's pinned verification keys and `@selfxyz/core` version remain trust-critical (a compromised/buggy bridge could accept bad proofs).
...
8. **Privacy of demographic disclosures.** Even age band + region can be deanonymizing in small populations. For low-population regions or rare demographic intersections, Chorum should aggregate or suppress breakdowns below a minimum cohort size.
...
- Should Chorum require the same passport-scoped nullifier to be used across re-registrations (e.g., if a user loses their device), or accept a new nullifier from the same passport? The former is more Sybil-resistant; the latter is more user-friendly.
